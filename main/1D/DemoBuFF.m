...
FilePath = './main/1D/images/';     % Burst Directory Path: If you are using your own burst, change the path here
...
SlopeSettings = [-4 4 2];
tic
[BurstFeature, BurstDescriptor] = BuFF1D(Burst, SlopeSettings, PeakThresh, Octaves, Levels);
toc 
...
hold on
MinSlope = min(BurstFeature(:,5));
SlopeRange = max(1,max(BurstFeature(:,5))-MinSlope);
