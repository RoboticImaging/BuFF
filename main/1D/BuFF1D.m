...
function [BurstFeature,BurstDescriptor] = BuFF1D(Burst, SlopeSettings, PeakThresh, Octaves, Levels)
addpath('common/')
...
for  islope = 1:NumSlope
         MotionImage = BurstShiftSum(DoubleBurst,SlopeSet(islope),0); 
         %for i = 1%:NumOctaves
         %SizeScale = 1/(2.^(i-1)); 
...
Stack = struct('x',0,'y',0,'Octave',0,'Level',0,'Offset',[0,0,0],'ScaleOctave',0, 'Speed', 0);
Index = 1;

for islope = 1:NumSlope
        for i = 2:NumOctaves
            [Height, Width] = size(BuFFPyramid{i}(:,:,1,1));    
            BurstFeatureStack = BuFFPyramid{i};                      
            BuFFStack = BurstFeatureStack(:,:,:,islope);

            for j = 2:ScaleSteps+1                               
                    BuFFImage = BurstFeatureStack(:,:,j,islope); 
                    ...
                    for x = OutlierPixel+1:Height-OutlierPixel
                        for y = OutlierPixel+1:Width-OutlierPixel
                            ...
                            if(abs(BuFFImage(x,y)) > PeakThresholdInitialization)
                                if(FindExtrema(BurstFeatureStack,j,islope,x,y,NumSlope))

                                        BurstKeyPoints = BurstKeyPointLocalization(BuFFStack,Height,Width,i,j,islope,x,y,OutlierPixel,PeakThreshold,Count);
...
Features = struct('Index',0,'x',0,'y',0,'Scale',0,'Orientation',0,'Slope',0,'Octave',0,'Layer',0, 'Descriptor',[]);
FeatureIndex = 1; 
...
for e = 1:StackLength
    ...
    OrientationHistogram = HistogramGeneration(MotionScalePyramid{KeyPoints.Octave}(:,:,KeyPoints.Level,KeyPoints.Speed),KeyPoints.x,KeyPoints.y,OrientationBins,round(3*GradientMagnitude),GradientMagnitude);
...
if (FeatureIndex == 0)
    BurstFeature = [0 0 0 0 0];
end

for FeatureIndex = 1:FeatureLength
    FeaturesSet = LocalFeatures(FeatureIndex);
    BurstKeyPoints = LocalStack(FeaturesSet.Index);
    ScaleMotionImage = LocalGaussianPyramid{BurstKeyPoints.Octave}(:,:,BurstKeyPoints.Level,BurstKeyPoints.Speed);
...
for i = 1:FeatureLength
    ...
    FeaturesSet(i,5) = LocalFeatures(FeatureOrder(i)).Slope;
end
end
